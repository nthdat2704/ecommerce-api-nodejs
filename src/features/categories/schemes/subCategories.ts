...
const subCategorySchema: ObjectSchema = Joi.object().keys({
  ...
  mainCategory: Joi.number().integer().required().min(1).messages({
    'string.base': 'mainCategory must be of type number',
    'string.min': 'Invalid mainCategory length',
    'string.empty': 'mainCategory is a required field'
  })
});
